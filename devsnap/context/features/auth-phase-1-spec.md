...
- Install NextAuth v5 (`next-auth@beta`) check if beta is latest now and `@auth/prisma-adapter`
...
- Use `next-auth@beta` (not `@latest` which installs v4) also check if beta is latest now
